...
public class AndroidPlatformSupport extends PlatformSupport {
	...
	public void updateDisplaySize() {
		...
		AndroidGame.instance.setRequestedOrientation(
				landscape ? ActivityInfo.SCREEN_ORIENTATION_SENSOR_LANDSCAPE : ActivityInfo.SCREEN_ORIENTATION_SENSOR_PORTRAIT);
			...
			public void run() {
				...
				if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
					if (SPDSettings.fullscreen()) {
						AndroidGame.instance.getWindow().getDecorView()
								.setSystemUiVisibility(View.SYSTEM_UI_FLAG_LAYOUT_STABLE | View.SYSTEM_UI_FLAG_LAYOUT_HIDE_NAVIGATION
										| View.SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN | View.SYSTEM_UI_FLAG_HIDE_NAVIGATION
										| View.SYSTEM_UI_FLAG_FULLSCREEN | View.SYSTEM_UI_FLAG_IMMERSIVE_STICKY);
					} else {
						AndroidGame.instance.getWindow().getDecorView().setSystemUiVisibility(View.SYSTEM_UI_FLAG_LAYOUT_STABLE);
					}
				}
			}
		});
	...
	@Override
	public void promptTextInput(final String title, final String hintText, final int maxLen, final boolean multiLine,
			final String posTxt, final String negTxt, final TextCallback callback) {
	...
	public void setupFontGenerators(int pageSize, boolean systemfont) {
		...
		if (Gdx.files.absolute("/system/fonts/NotoSansCJK-Regular.ttc").exists()) {
			...
			KRFontGenerator = SCFontGenerator = JPFontGenerator = new FreeTypeFontGenerator(
		...
		// would be nice to use RGBA4444 to save memory, but this causes problems on
		// some gpus =S
	...
	private static Pattern SCMatcher = Pattern
			.compile("\\p{InCJK_Unified_Ideographs}|\\p{InCJK_Symbols_and_Punctuation}|\\p{InHalfwidth_and_Fullwidth_Forms}");
