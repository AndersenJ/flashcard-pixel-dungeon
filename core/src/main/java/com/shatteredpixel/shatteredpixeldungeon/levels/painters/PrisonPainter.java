...
public class PrisonPainter extends RegularPainter {
	...
	protected void decorate(Level level, List<Room> rooms) {
		...
		for (int i = 0; i < w; i++) {
			if (map[i] == Terrain.WALL && (map[i + w] == Terrain.EMPTY || map[i + w] == Terrain.EMPTY_SP)
					&& Random.Int(6) == 0) {
		...
		for (int i = w; i < l - w; i++) {
			if (map[i] == Terrain.WALL && map[i - w] == Terrain.WALL
					&& (map[i + w] == Terrain.EMPTY || map[i + w] == Terrain.EMPTY_SP) && Random.Int(3) == 0) {
