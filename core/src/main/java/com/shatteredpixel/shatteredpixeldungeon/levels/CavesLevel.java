...
public class CavesLevel extends RegularLevel {
	...
	protected Painter painter() {
		return new CavesPainter().setWater(feeling == Feeling.WATER ? 0.85f : 0.30f, 6)
				.setGrass(feeling == Feeling.GRASS ? 0.65f : 0.15f, 3).setTraps(nTraps(), trapClasses(), trapChances());
	}
	...
	protected Class<?>[] trapClasses() {
		return new Class[] { BurningTrap.class, PoisonDartTrap.class, FrostTrap.class, StormTrap.class, CorrosionTrap.class,
				GrippingTrap.class, ExplosiveTrap.class, RockfallTrap.class, GuardianTrap.class, ConfusionTrap.class,
				SummoningTrap.class, WarpingTrap.class, PitfallTrap.class };
	}

	@Override
	protected float[] trapChances() {
		return new float[] { 8, 8, 8, 8, 8, 4, 4, 4, 4, 2, 2, 2, 1 };
	}
		...
		public void update() {
			...
			if (visible = (pos < Dungeon.level.heroFOV.length && Dungeon.level.heroFOV[pos])) {
				...
				if ((delay -= Game.elapsed) <= 0) {
					...
					((Sparkle) recycle(Sparkle.class)).reset(p.x + Random.Float(DungeonTilemap.SIZE),
