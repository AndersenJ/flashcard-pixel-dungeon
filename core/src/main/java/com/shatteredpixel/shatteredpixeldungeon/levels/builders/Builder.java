...
public abstract class Builder {
	...
	// returns a rectangle representing the maximum amount of free space from a
	...
	protected static Rect findFreeSpace(Point start, List<Room> collision, int maxSize) {
		...
		// shallow copy
		List<Room> colliding = new ArrayList<>(collision);
		do {
			...
			while (it.hasNext()) {
				Room room = it.next();
				// if not colliding
				if (room.isEmpty() || Math.max(space.left, room.left) >= Math.min(space.right, room.right)
	...
	// returns the angle in degrees made by the centerpoints of 2 rooms, with 0
	// being straight up.
	...
	// Attempts to place a room such that the angle between the center of the
	// previous room
	// and it matches the given angle ([0-360), where 0 is straight up) as closely
	// as possible.
	// Note that getting an exactly correct angle is harder the closer that angle is
	// to diagonal.
	// Returns the exact angle between the centerpoints of the two rooms, or -1 if
	// placement fails.
	protected static float placeRoom(List<Room> collision, Room prev, Room next, float angle) {
		...
		// using the line equation, we find the point along the prev room where the line
		// exists
		Point start;
		int direction;
		if (Math.abs(m) >= 1) {
		...
		// find the ideal center for this new room using the line equation and known
		// dimensions
