...
public abstract class SecretRoom extends SpecialRoom {
	...
	private static final List<Class<? extends SecretRoom>> ALL_SECRETS = new ArrayList<>(
			Arrays.asList(SecretGardenRoom.class, SecretLaboratoryRoom.class, SecretLibraryRoom.class, SecretLarderRoom.class,
					SecretWellRoom.class, SecretRunestoneRoom.class, SecretArtilleryRoom.class, SecretChestChasmRoom.class,
					SecretHoneypotRoom.class, SecretHoardRoom.class, SecretMazeRoom.class, SecretSummoningRoom.class));

	public static List<Class<? extends SecretRoom>> runSecrets = new ArrayList<>();
	...
	public static void restoreRoomsFromBundle(Bundle bundle) {
		...
		if (bundle.contains(ROOMS)) {
			Stream.of(bundle.getClassArray(ROOMS)).filter(type -> type != null)
					.map(type -> (Class<? extends SecretRoom>) type.asSubclass(SecretRoom.class))
					.forEach(type -> runSecrets.add(type));
