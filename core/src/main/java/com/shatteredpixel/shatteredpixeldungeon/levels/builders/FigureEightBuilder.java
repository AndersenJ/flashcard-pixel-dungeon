...
public class FigureEightBuilder extends RegularBuilder {
	...
	// increasing the exponent will increase the the curvature, making the loop more
	// oval shaped.
	...
	// a common example, setting to 0.25 will make for a short fat oval instead of a
	// long one.
	...
	private float targetAngle(float percentAlong) {
		...
		return 360f
				* (float) (curveIntensity * curveEquation(percentAlong) + (1 - curveIntensity) * (percentAlong) - curveOffset);
	}

	private double curveEquation(double x) {
		return Math.pow(4, 2 * curveExponent) * (Math.pow((x % 0.5f) - 0.25, 2 * curveExponent + 1)) + 0.25
				+ 0.5 * Math.floor(2 * x);
	}
	...
	public List<Room> build(List<Room> rooms) {
		...
		// TODO might want to make this able to work without an exit. Probably a random
		// room would be landmark and the landmark room would become exit
		...
		if (entrance != null)
			firstLoop.add((firstLoop.size() + 1) / 2, entrance);
		...
		if (exit != null)
			secondLoop.add((secondLoop.size() + 1) / 2, exit);
		...
		// should just write a general function for stitching two rooms together in
		// builder
		...
		List<Room> branchable = new ArrayList<>(firstLoop);
	...
	protected float randomBranchAngle(Room r) {
		...
		if (center == null)
			...
			// generate four angles randomly and return the one which points closer to the
			// center
