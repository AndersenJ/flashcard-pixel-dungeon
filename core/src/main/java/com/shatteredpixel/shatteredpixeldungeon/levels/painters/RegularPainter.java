...
public abstract class RegularPainter extends Painter {
	...
	@Override
	public boolean paint(Level level, List<Room> rooms) {
	...
	private void placeDoors(Room r) {
		for (Room n : r.connected.keySet()) {
			...
			if (door == null) {
				...
				List<Point> doorSpots = new ArrayList<>();
				for (Point p : i.getPoints()) {
					if (r.canConnect(p) && n.canConnect(p))
						doorSpots.add(p);
				}
				if (doorSpots.isEmpty()) {
					ShatteredPixelDungeon.reportException(new RuntimeException(
							"Could not place a door! " + "r=" + r.getClass().getSimpleName() + " n=" + n.getClass().getSimpleName()));
					continue;
				}
	...
	protected void paintGrass(Level l, List<Room> rooms) {
		...
		List<Integer> grassCells = new ArrayList<>();
		...
		// Adds chaos to grass height distribution. Ratio of high grass depends on fill
		// and smoothing
		// Full range is 8.3% to 75%, but most commonly (20% fill with 3 smoothing) is
		// around 60%
		// low smoothing, or very low fill, will begin to push the ratio down, normally
		// to 50-30%
