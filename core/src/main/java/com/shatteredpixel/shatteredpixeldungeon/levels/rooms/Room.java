...
public abstract class Room extends Rect implements Graph.Node, Bundlable {
	...
	public List<Room> neigbours = new ArrayList<>();
	...
	// the same value over multiple calls, even if there's some randomness
	// initially.
	public int minWidth() {
		return -1;
	}
	...
	protected boolean setSize(int minW, int maxW, int minH, int maxH) {
		if (minW < minWidth() || maxW > maxWidth() || minH < minHeight() || maxH > maxHeight() || minW > maxW
				|| minH > maxH) {
			return false;
		} else {
			// subtract one because rooms are inclusive to their right and bottom sides
			resize(Random.NormalIntRange(minW, maxW) - 1, Random.NormalIntRange(minH, maxH) - 1);
			return true;
		}
	}

	// Width and height are increased by 1 because rooms are inclusive to their
	...
	public Point center() {
		return new Point((left + right) / 2 + (((right - left) % 2) == 1 ? Random.Int(2) : 0),
	...
	public boolean connect(Room room) {
		if ((neigbours.contains(room) || addNeigbour(room)) && !connected.containsKey(room) && canConnect(room)) {
	...
	public final List<Point> waterPlaceablePoints() {
	...
	public final List<Point> grassPlaceablePoints() {
		List<Point> points = new ArrayList<>();
		for (int i = left; i <= right; i++) {
			for (int j = top; j <= bottom; j++) {
				Point p = new Point(i, j);
	...
	public final List<Point> trapPlaceablePoints() {
		List<Point> points = new ArrayList<>();
		for (int i = left; i <= right; i++) {
			for (int j = top; j <= bottom; j++) {
				Point p = new Point(i, j);
	...
	public final List<Point> charPlaceablePoints(Level l) {
		List<Point> points = new ArrayList<>();
		for (int i = left; i <= right; i++) {
			for (int j = top; j <= bottom; j++) {
				Point p = new Point(i, j);
	...
	public Collection<Room> edges() {
		...
		for (Room r : connected.keySet()) {
			...
			// for the purposes of path building, ignore all doors that are locked, blocked,
			// or hidden
			if (d.type == Door.Type.EMPTY || d.type == Door.Type.TUNNEL || d.type == Door.Type.UNLOCKED
