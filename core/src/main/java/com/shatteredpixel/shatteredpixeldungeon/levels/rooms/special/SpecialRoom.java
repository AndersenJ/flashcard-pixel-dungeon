...
public abstract class SpecialRoom extends Room {
	...
	private static final List<Class<? extends SpecialRoom>> ALL_SPEC = new ArrayList<>(Arrays.asList(WeakFloorRoom.class,
	...
	public static List<Class<? extends Room>> runSpecials = new ArrayList<>();
	public static List<Class<? extends Room>> floorSpecials = new ArrayList<>();

	private static int pitNeededDepth = -1;

	public static void initForRun() {
		runSpecials = new ArrayList<Class<? extends Room>>(ALL_SPEC);
	...
	public static void initForFloor() {
		floorSpecials = new ArrayList<Class<? extends Room>>(runSpecials);
	...
	public static void restoreRoomsFromBundle(Bundle bundle) {
		...
		if (bundle.contains(ROOMS)) {
			Stream.of(bundle.getClassArray(ROOMS)).filter(type -> (type != null && type != LaboratoryRoom.class))
					.map(type -> (Class<? extends Room>) type.asSubclass(Room.class)).forEach(type -> runSpecials.add(type));
		} else {
			initForRun();
