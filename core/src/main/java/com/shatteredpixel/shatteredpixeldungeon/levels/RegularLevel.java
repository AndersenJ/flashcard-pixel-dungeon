...
public abstract class RegularLevel extends Level {
	...
	protected boolean build() {
		...
		do {
			...
			rooms = builder.build(new ArrayList<Room>(initRooms));
		} while (rooms == null);
	...
	protected Builder builder() {
		return new LoopBuilder().setLoopShape(2, Random.Float(0.4f, 0.7f), Random.Float(0f, 0.5f));
	}
	...
	protected void createMobs() {
		...
		List<Room> stdRooms = new ArrayList<>();
		for (Room room : rooms) {
	...
	public int randomRespawnCell() {
		...
		while (true) {
			...
			if (!heroFOV[cell] && Actor.findChar(cell) == null && passable[cell]
					&& room.canPlaceCharacter(cellToPoint(cell), this) && cell != exit) {
				return cell;
			}
	...
	protected int randomDropCell() {
		while (true) {
			...
			if (room != null && room != roomEntrance) {
				...
				if (passable[pos] && pos != exit && heaps.get(pos) == null) {

					Trap t = traps.get(pos);

					// items cannot spawn on traps which destroy items
					if (t == null || !(t instanceof BurningTrap || t instanceof BlazingTrap || t instanceof ChillingTrap
							|| t instanceof FrostTrap || t instanceof ExplosiveTrap || t instanceof DisintegrationTrap)) {
	...
	public void restoreFromBundle(Bundle bundle) {
		...
		rooms = new ArrayList<>(
				bundle.getCollection("rooms").stream().map(room -> (Room) room).collect(Collectors.toList()));
		for (Room r : rooms) {
			r.onLevelLoad(this);
