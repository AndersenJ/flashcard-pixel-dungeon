...
public class PerimeterRoom extends ConnectionRoom {
	...
	private static int distanceBetweenPoints(Room r, Point a, Point b) {
		// on the same side
		if (((a.x == r.left || a.x == r.right) && a.y == b.y) || ((a.y == r.top || a.y == r.bottom) && a.x == b.x)) {
	...
	private static void fillBetweenPoints(Level level, Room r, Point from, Point to, int floor) {
		...
		if (from.y == r.top + 1 || from.y == r.bottom - 1) {
			...
			if (spaceBetween(r.left, from.x) + spaceBetween(r.left, to.x) <= spaceBetween(r.right, from.x)
					+ spaceBetween(r.right, to.x)) {
			...
			if (spaceBetween(r.top, from.y) + spaceBetween(r.top, to.y) <= spaceBetween(r.bottom, from.y)
					+ spaceBetween(r.bottom, to.y)) {
