...
public class ShopRoom extends SpecialRoom {
	...
	protected static List<Item> generateItems() {
		...
		List<Item> itemsToSpawn = new ArrayList<>();
		...
		for (int i = 0; i < 2; i++)
			itemsToSpawn.add(Random.Int(2) == 0 ? Generator.random(Generator.Category.POTION)
		...
		// hard limit is 63 items + 1 shopkeeper, as shops can't be bigger than 8x8=64
		// internally
	...
	protected static Bag ChooseBag(Belongings pack) {
		...
		// find the best bag to drop. This does give a preference to later bags, if
		// counts are equal
		int bestBagIdx = 0;
		for (int i = 1; i <= 3; i++) {
