...
public final class ShadowCaster {
	...
	static {
		...
		for (int i = 1; i <= MAX_DISTANCE; i++) {
			...
			for (int j = 1; j <= i; j++) {
				...
				rounding[i][j] = (int) Math.min(j, Math.round((i + 0.5) * Math.cos(Math.asin(j / (i + 0.5)))));
			}
		}
	}
	...
	private static void scanOctant(int distance, boolean[] fov, boolean[] blocking, int row, int x, int y, double lSlope,
			double rSlope, int mX, int mY, boolean mXY) {
		...
		// calculations are offset by 0.5 because FOV is coming from the center of the
		// source cell

		// for each row, starting with the current one
		for (; row <= distance; row++) {

			// we offset by slightly less than 0.5 to account for slopes just touching a
			// cell
			if (lSlope == 0)
				start = 0;
			else
			...
			if (rSlope == 1)
				...
				end = Math.min(rounding[distance][row], (int) Math.ceil((row + 0.5) * rSlope - 0.499));
