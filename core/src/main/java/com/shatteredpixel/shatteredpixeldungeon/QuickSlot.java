...
public class QuickSlot {
	...
	// note that the current max size is coded at 4, due to UI constraints, but it
	 ...
	 * index is not, so if we bundle both the placeholders (which preserves their
	 * order) and an array telling us where the placeholders are, we can reconstruct
	 * them perfectly.
	 */

	public void storePlaceholders(Bundle bundle) {
		List<Item> placeholders = new ArrayList<>(SIZE);
