...
public class CellSelector extends ScrollArea {
	...
	protected void onClick(PointerEvent event) {
		if (dragging) {
			...
			select(((DungeonTilemap) target).screenToTile((int) event.current.x, (int) event.current.y, true));
		}
	}
	...
	private float zoom(float value) {
		...
		// This is important as characters are centered on a 16x16 tile, but may have
		// any sprite size
		// This can lead to none-whole coordinate, which need to be aligned with the
		// zoom
	...
	protected void onDrag(PointerEvent event) {
		...
		if (pinching) {
			...
			camera.zoom(GameMath.gate(PixelScene.minZoom, zoom - (zoom % 0.1f), PixelScene.maxZoom));
