...
public class PixelScene extends Scene {
	...
	public void create() {
		...
		if (pixelFont == null) {
			...
			// 3x5 (6)
			pixelFont = Font.colorMarked(BitmapCache.get(Assets.PIXELFONT), 0x00000000, BitmapText.Font.LATIN_FULL);
		...
		// anticipate that
		if (Messages.lang() == Languages.KOREAN || Messages.lang() == Languages.CHINESE
	...
	private static List<Class<? extends Window>> savedWindows = new ArrayList<>();
	...
	public void saveWindows() {
		...
		for (Gizmo g : members.toArray(new Gizmo[0])) {
			if (g instanceof Window) {
				savedWindows.add((Class<? extends Window>) g.getClass().asSubclass(Window.class));
			}
		}
	}
	...
	/**
	 * These methods align UI elements to device pixels. e.g. if we have a scale of
	 * 3x then valid positions are #.0, #.33, #.67
	 */
	...
	private static class PixelCamera extends Camera {
		...
		public PixelCamera(float zoom) {
			super((int) (Game.width - Math.ceil(Game.width / zoom) * zoom) / 2,
					(int) (Game.height - Math.ceil(Game.height / zoom) * zoom) / 2, (int) Math.ceil(Game.width / zoom),
