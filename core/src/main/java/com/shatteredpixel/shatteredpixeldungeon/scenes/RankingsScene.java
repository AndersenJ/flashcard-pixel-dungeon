...
public class RankingsScene extends PixelScene {
	...
	public void create() {
		...
		if (Rankings.INSTANCE.records.size() > 0) {
			...
			// attempts to give each record as much space as possible, ideally as much space
			// as portrait mode
			float rowHeight = GameMath.gate(ROW_HEIGHT_MIN, (uiCamera.height - 26) / Rankings.INSTANCE.records.size(),
					ROW_HEIGHT_MAX);
			...
			if (Rankings.INSTANCE.totalNumber >= Rankings.TABLE_SIZE) {
				...
				label.text(
						Messages.get(this, "total") + " _" + Rankings.INSTANCE.wonNumber + "_/" + Rankings.INSTANCE.totalNumber);
				add(label);

				label.setPos((w - label.width()) / 2, h - label.height() - 2 * GAP);
				align(label);
			...
			noRec.setPos((w - noRec.width()) / 2, (h - noRec.height()) / 2);
			align(noRec);
			add(noRec);
