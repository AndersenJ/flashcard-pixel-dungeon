...
public class WallBlockingTilemap extends Tilemap {
	...
	public synchronized void updateMapCell(int cell) {
		...
		// TODO should doors be considered? currently the blocking is a bit permissive
		// around doors
		...
		if (!wall(cell)) {
			...
			} else if (!fogHidden(cell + mapWidth) && (fogHidden(cell - 1) || wall(cell - 1))
					&& (fogHidden(cell + 1) || wall(cell + 1)) && (fogHidden(cell - 1 + mapWidth) || wall(cell - 1 + mapWidth))
			...
			if (!wall(cell + mapWidth)) {
				...
				// - the cell above, above-left, or above-right is not a wall, visible, and has
				// a wall below
				...
				if (wall(cell - 1 - mapWidth) && wall(cell - mapWidth) && wall(cell + 1 - mapWidth)) {
					...
					if (!fogHidden(cell + mapWidth) || (!wall(cell - 1) && !fogHidden(cell - 1))
				...
				if (!fogHidden(cell - mapWidth) || !fogHidden(cell) || !fogHidden(cell + mapWidth)) {
					...
					// right side
					if (((cell + 1) % mapWidth == 0) || (!wall(cell + 1) && !fogHidden(cell + 1 - mapWidth))
							|| (!wall(cell + 1) && !fogHidden(cell + 1))
							|| (!wall(cell + 1 + mapWidth) && !fogHidden(cell + 1 + mapWidth))) {
						// do nothing
					} else {
					...
					// left side
					if ((cell % mapWidth == 0) || (!wall(cell - 1) && !fogHidden(cell - 1 - mapWidth))
							|| (!wall(cell - 1) && !fogHidden(cell - 1))
							|| (!wall(cell - 1 + mapWidth) && !fogHidden(cell - 1 + mapWidth))) {
						// do nothing
					} else {
	...
	private boolean fogHidden(int cell) {
		if (!Dungeon.level.visited[cell] && !Dungeon.level.mapped[cell]) {
			return true;
		} else if (wall(cell) && cell + mapWidth < size && !wall(cell + mapWidth) && !Dungeon.level.visited[cell + mapWidth]
