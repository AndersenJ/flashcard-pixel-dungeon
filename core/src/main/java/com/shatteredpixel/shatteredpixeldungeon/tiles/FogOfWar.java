...
public class FogOfWar extends Image {
	...
	private static final int FOG_COLORS[][] = new int[][] { {
			...
			0x00000000, 0x00000000 // 1 and 2 brightness
					...
					// visited
					0xDD000000, 0xBB000000, 0x99000000, 0x77000000, 0x55000000 },
			{
					// mapped
					0xDD221108, 0xBB442211, 0x99663319, 0x77884411, 0x55AA552A },
			{
					// invisible
	...
	/*
	 * TODO currently the center of each fox pixel is aligned with the inside of a
	 * cell might be possible to create a better fog effect by aligning them with
	 * edges of a cell, similar to the existing fog effect in vanilla (although
	 * probably with more precision) the advantage here is that it may be possible
	 * to totally eliminate the tile blocking map
	 */
	...
	public synchronized void updateFog(int cell, int radius) {
		Rect update = new Rect((cell % mapWidth) - radius, (cell / mapWidth) - radius,
				(cell % mapWidth) - radius + 1 + 2 * radius, (cell / mapWidth) - radius + 1 + 2 * radius);
					...
					if (wall(cell)) {
						...
						if (cell + mapWidth >= mapLength) {
							...
							if (cell % mapWidth != 0) {
								...
								// picks the darkest fog between current tile, left, and below-left(if left is a
								// wall).
								if (wall(cell - 1)) {
									...
									if (wall(cell + mapWidth - 1)) {
										...
										fillLeft(fog, j, i, FOG_COLORS[Math.max(getCellFog(cell),
												Math.max(getCellFog(cell + mapWidth - 1), getCellFog(cell - 1)))][brightness]);
									}
							...
							if ((cell + 1) % mapWidth != 0) {
								...
								// picks the darkest fog between current tile, right, and below-right(if right
								// is a wall).
								if (wall(cell + 1)) {
									...
									if (wall(cell + mapWidth + 1)) {
										...
										fillRight(fog, j, i, FOG_COLORS[Math.max(getCellFog(cell),
												Math.max(getCellFog(cell + mapWidth + 1), getCellFog(cell + 1)))][brightness]);
									}
