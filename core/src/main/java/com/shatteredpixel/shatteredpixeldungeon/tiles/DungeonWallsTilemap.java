	...
	protected int getTileVisual(int pos, int tile, boolean flat) {
		...
		if (DungeonTileSheet.wallStitcheable(tile)) {
			if (pos + mapWidth < size && !DungeonTileSheet.wallStitcheable(map[pos + mapWidth])) {
				...
				return DungeonTileSheet.stitchInternalWallTile(tile, (pos + 1) % mapWidth != 0 ? map[pos + 1] : -1,
		...
		if (pos + mapWidth < size && DungeonTileSheet.wallStitcheable(map[pos + mapWidth])) {
			...
			return DungeonTileSheet.stitchWallOverhangTile(tile, (pos + 1) % mapWidth != 0 ? map[pos + 1 + mapWidth] : -1,
					map[pos + mapWidth], pos % mapWidth != 0 ? map[pos - 1 + mapWidth] : -1);
		...
		} else if (pos + mapWidth < size
				&& (map[pos + mapWidth] == Terrain.STATUE || map[pos + mapWidth] == Terrain.STATUE_SP)) {
