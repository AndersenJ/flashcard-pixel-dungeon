...
public class DungeonTileSheet {
	...
	public static HashSet<Integer> waterStitcheable = new HashSet<>(
			Arrays.asList(Terrain.EMPTY, Terrain.GRASS, Terrain.EMPTY_WELL, Terrain.ENTRANCE, Terrain.EXIT, Terrain.EMBERS,
					Terrain.BARRICADE, Terrain.HIGH_GRASS, Terrain.FURROWED_GRASS, Terrain.SECRET_TRAP, Terrain.TRAP,
					Terrain.INACTIVE_TRAP, Terrain.EMPTY_DECO, Terrain.SIGN, Terrain.WELL, Terrain.STATUE, Terrain.ALCHEMY,
					Terrain.DOOR, Terrain.OPEN_DOOR, Terrain.LOCKED_DOOR));

	// +1 for ground above, +2 for ground right, +4 for ground below, +8 for ground
	// left.
	...
	// tiles that can stitch with chasms (from above), and which visual represents
	// the stitching
	...
	private static int[] wallStitcheable = new int[] { Terrain.WALL, Terrain.WALL_DECO, Terrain.SECRET_DOOR,
	...
	private static int[] doorTiles = new int[] { Terrain.DOOR, Terrain.LOCKED_DOOR, Terrain.OPEN_DOOR };
	...
	// +1 for open right, +2 for open right-below, +4 for open left-below, +8 for
	// open left.
	...
	// These visuals always directly represent a game tile with no stitching
	// required
	...
	// These visuals directly represent game tiles (no stitching) when terrain is
	// being shown as flat
	...
	// These alt visuals will trigger 50% of the time (45% of the time if a rare alt
	// is also present)
	...
	// These alt visuals trigger 5% of the time (and also override common alts when
	// they show up)
