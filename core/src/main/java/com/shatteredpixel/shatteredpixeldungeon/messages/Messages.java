...
public class Messages {
	 ...
	 * resourcebundle.containskey(), secondly so I can read in and combine multiple
	 * properties files, resulting in a more clean structure for organizing all the
	 * strings, instead of one big file.
	 * 
	...
	private static String[] prop_files = new String[] { "com.shatteredpixel.shatteredpixeldungeon.messages.actors.actors",
	...
	public static String get(Class<?> c, String k, Object... args) {
		...
		if (strings.containsKey(key.toLowerCase(Locale.ENGLISH))) {
			if (args.length > 0)
				...
				return strings.get(key.toLowerCase(Locale.ENGLISH));
		} else {
			// this is so child classes can inherit properties from their parents.
			// in cases where text is commonly grabbed as a utility from classes that aren't
			// mean to be instantiated
			// (e.g. flavourbuff.dispTurns()) using .class directly is probably smarter to
