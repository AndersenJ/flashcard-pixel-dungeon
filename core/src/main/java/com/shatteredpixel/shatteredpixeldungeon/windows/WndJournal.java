...
public class WndJournal extends WndTabbed {
	...
	public WndJournal() {
		...
		Tab[] tabs = { new IconTab(new ItemSprite(ItemSpriteSheet.GUIDE_PAGE, null)) {
	...
	private static class GuideTab extends Component {
		...
		private List<GuideItem> pages = new ArrayList<>();
	...
	public static class AlchemyTab extends Component {
		...
		private ScrollPane list;
		private List<QuickRecipe> recipes = new ArrayList<>();
		...
		private void updateList() {
			...
			int w;
			List<QuickRecipe> toAddThisRow = new ArrayList<>();
			while (!toAdd.isEmpty()) {
				if (toAdd.get(0) == null) {
					toAdd.remove(0);
					top += 6;
	...
	private static class NotesTab extends Component {
		...
		private void updateList() {
			...
			// Keys
			List<Notes.KeyRecord> keys = Notes.getRecords(Notes.KeyRecord.class);
			if (!keys.isEmpty()) {
				ColorBlock line = new ColorBlock(width(), 1, 0xFF222222);
				line.y = pos;
				content.add(line);
			...
			for (Notes.Record rec : keys) {
				ListItem item = new ListItem(Icons.get(Icons.DEPTH), Messages.titleCase(rec.desc()), rec.depth());
				item.setRect(0, pos, width(), ITEM_HEIGHT);
				content.add(item);
			...
			// Landmarks
			List<Notes.LandmarkRecord> landmarks = Notes.getRecords(Notes.LandmarkRecord.class);
	...
	private static class CatalogTab extends Component {
		...
		protected void layout() {
			...
			for (int i = 0; i < NUM_BUTTONS; i++) {
				itemButtons[i].setRect((i % perRow) * (buttonWidth), (i / perRow) * (ITEM_HEIGHT), buttonWidth, ITEM_HEIGHT);
		...
		private void updateList() {
			...
			for (Class<? extends Item> itemClass : itemClasses) {
				CatalogItem item = new CatalogItem(Reflection.newInstance(itemClass), known.get(itemClass),
						Catalog.isSeen(itemClass));
				item.setRect(0, pos, width, ITEM_HEIGHT);
				content.add(item);
				items.add(item);
			...
			public boolean onClick(float x, float y) {
				if (inside(x, y) && seen) {
					if (item instanceof ClassArmor) {
						GameScene.show(new WndTitledMessage(new Image(icon), Messages.titleCase(item.trueName()), item.desc()));
					} else {
