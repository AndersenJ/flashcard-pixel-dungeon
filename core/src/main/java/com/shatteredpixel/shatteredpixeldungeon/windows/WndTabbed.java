...
public class WndTabbed extends Window {
	...
	public void resize(int w, int h) {
		...
		List<Tab> tabs = new ArrayList<>(this.tabs);
		this.tabs.clear();
	...
	public void layoutTabs() {
		...
		for (int i = 0; i < tabs.size(); i++) {
			...
			tabs.get(i).setPos(i == 0 ? -chrome.marginLeft() + 1 : tabs.get(i - 1).right() + spacing, height);
		}
	...
	protected class LabeledTab extends Tab {
		...
		protected void layout() {
			...
			btLabel.setPos(x + (width - btLabel.width()) / 2, y + (height - btLabel.height()) / 2 - (selected ? 1 : 3));
