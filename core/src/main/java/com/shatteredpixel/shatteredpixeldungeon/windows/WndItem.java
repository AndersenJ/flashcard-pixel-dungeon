...
public class WndItem extends Window {
	...
	public WndItem(final WndBag owner, final Item item, final boolean options) {
		...
		while (SPDSettings.landscape() && info.height() > 100 && width < WIDTH_MAX) {
			width += 20;
			info.maxWidth(width);
		}
	...
	private static float layoutButtons(List<RedButton> buttons, float width, float y) {
		List<RedButton> curRow = new ArrayList<>();
		...
		while (!buttons.isEmpty()) {
			...
			// layout current row. Currently forces a max of 3 buttons but can work with
			// more
			if (buttons.isEmpty() || widthLeftThisRow <= 0 || curRow.size() >= 3) {
				...
				// while we still have space in this row, find the shortest button(s) and extend
				// them
				while (widthLeftThisRow > 0) {

					List<RedButton> shortest = new ArrayList<>();
