...
public class MagicMissile extends Emitter {
	...
	public void reset(int type, int from, int to, Callback callback) {
		reset(type, DungeonTilemap.raisedTileCenterToWorld(from), DungeonTilemap.raisedTileCenterToWorld(to), callback);
	}

	public void reset(int type, Visual from, Visual to, Callback callback) {
		reset(type, from.center(), to.center(), callback);
	}

	public void reset(int type, Visual from, int to, Callback callback) {
		reset(type, from.center(), DungeonTilemap.raisedTileCenterToWorld(to), callback);
	}
	...
	// convenience method for the common case of a bolt going from a character to a
	// tile or enemy
