...
public class CircleArc extends Visual {
	...
	public CircleArc(int triangles, float radius) {
		...
		vertices = ByteBuffer.allocateDirect((nTris * 2 + 1) * 4 * (Float.SIZE / 8)).order(ByteOrder.nativeOrder())
				.asFloatBuffer();

		indices = ByteBuffer.allocateDirect(nTris * 3 * Short.SIZE / 8).order(ByteOrder.nativeOrder()).asShortBuffer();
