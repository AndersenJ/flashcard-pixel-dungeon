...
public class CursedWand {
	...
	private static void uncommonEffect(final Item origin, final Hero user, final Ballistica bolt,
			final Callback afterZap) {
		switch (Random.Int(4)) {
		...
		case 0:
			cursedFX(user, bolt, new Callback() {
				public void call() {
					...
					if (pos == Terrain.EMPTY || pos == Terrain.EMBERS || pos == Terrain.EMPTY_DECO || pos == Terrain.GRASS
							|| pos == Terrain.HIGH_GRASS || pos == Terrain.FURROWED_GRASS) {
		...
		case 0:
			cursedFX(user, bolt, new Callback() {
				public void call() {
					...
					if (ch != null && ch != user && !ch.properties().contains(Char.Property.BOSS)
	...
	private static void veryRareEffect(final Item origin, final Hero user, final Ballistica bolt,
			final Callback afterZap) {
		switch (Random.Int(4)) {
