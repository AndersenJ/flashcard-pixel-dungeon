...
public class RegrowthBomb extends Bomb {
	...
	public void explode(int cell) {
		...
		List<Integer> plantCandidates = new ArrayList<>();
		...
		for (int i = 0; i < PathFinder.distance.length; i++) {
			if (PathFinder.distance[i] < Integer.MAX_VALUE) {
				...
				} else if (Dungeon.level.map[i] == Terrain.EMPTY || Dungeon.level.map[i] == Terrain.EMBERS
						|| Dungeon.level.map[i] == Terrain.EMPTY_DECO || Dungeon.level.map[i] == Terrain.GRASS
