...
public class Lucky extends Weapon.Enchantment {
	...
	public int proc(Weapon weapon, Char attacker, Char defender, int damage) {
		...
		// lvl 1 ~ 12%
		// lvl 2 ~ 14%
		if (defender.HP <= damage && Random.Int(level + 40) >= 36) {
	...
	public static Item genLoot() {
		...
		if (roll < 0.6f) {
			...
			return Random.Int(2) == 0 ? Generator.random(Generator.Category.SEED)
					: Generator.random(Generator.Category.STONE);
		} else {
			return Random.Int(2) == 0 ? Generator.random(Generator.Category.POTION)
