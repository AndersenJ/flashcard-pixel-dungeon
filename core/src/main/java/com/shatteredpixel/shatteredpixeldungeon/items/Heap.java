...
public class Heap implements Bundlable {
	...
	public void explode() {
		...
		if (type != Type.HEAP) {
			...
			for (Item item : items.toArray(new Item[0])) {
				...
				} else if (!(item.level() > 0 || item.unique || (item instanceof Armor && ((Armor) item).checkSeal() != null)))
					items.remove(item);
	...
	public void restoreFromBundle(Bundle bundle) {
		...
		items = new LinkedList<>(
				bundle.getCollection(ITEMS).stream().map(item -> (Item) item).collect(Collectors.toList()));
		items.removeAll(Collections.singleton(null));

		// remove any document pages that either don't exist anymore or that the player
		// already has
		for (Item item : items.toArray(new Item[0])) {
