...
public class Armor extends EquipableItem {
	...
	public enum Augment {
		EVASION(1.5f, -1f), DEFENSE(-1.5f, 1f), NONE(0f, 0f);
	...
	// these are not used to process specific glyph effects, so magic immune doesn't
	// affect them
	...
	public static abstract class Glyph implements Bundlable {
		...
		private static final Class<?>[] common = new Class<?>[] { Obfuscation.class, Swiftness.class, Viscosity.class,
				Potential.class };

		private static final Class<?>[] uncommon = new Class<?>[] { Brimstone.class, Stone.class, Entanglement.class,
				Repulsion.class, Camouflage.class, Flow.class };

		private static final Class<?>[] rare = new Class<?>[] { Affection.class, AntiMagic.class, Thorns.class };

		private static final float[] typeChances = new float[] { 50, // 12.5% each
				40, // 6.67% each
				10 // 3.33% each
		};

		private static final Class<?>[] curses = new Class<?>[] { AntiEntropy.class, Corrosion.class, Displacement.class,
				Metabolism.class, Multiplicity.class, Stench.class, Overgrowth.class, Bulk.class };
		...
		public static Glyph randomCommon(Class<? extends Glyph>... toIgnore) {
			List<Class<?>> glyphs = new ArrayList<>(Arrays.asList(common));
		...
		public static Glyph randomUncommon(Class<? extends Glyph>... toIgnore) {
			List<Class<?>> glyphs = new ArrayList<>(Arrays.asList(uncommon));
			glyphs.removeAll(Arrays.asList(toIgnore));
			if (glyphs.isEmpty()) {
				return random();
			} else {
		...
		public static Glyph randomRare(Class<? extends Glyph>... toIgnore) {
			List<Class<?>> glyphs = new ArrayList<>(Arrays.asList(rare));
			glyphs.removeAll(Arrays.asList(toIgnore));
			if (glyphs.isEmpty()) {
				return random();
			} else {
		...
		public static Glyph randomCurse(Class<? extends Glyph>... toIgnore) {
			List<Class<?>> glyphs = new ArrayList<>(Arrays.asList(curses));
			glyphs.removeAll(Arrays.asList(toIgnore));
			if (glyphs.isEmpty()) {
				return random();
			} else {
