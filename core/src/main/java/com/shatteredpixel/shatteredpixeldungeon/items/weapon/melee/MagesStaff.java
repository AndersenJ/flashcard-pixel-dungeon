...
public class MagesStaff extends MeleeWeapon {
	...
	public int reachFactor(Char owner) {
		...
		if (owner instanceof Hero && wand instanceof WandOfDisintegration
	...
	public Item imbueWand(Wand wand, Char owner) {
		...
		if (wand.level() >= this.level() && this.level() > (curseInfusionBonus ? 1 : 0))
			targetLevel++;
		...
		public void onSelect(final Item item) {
			if (item != null) {
				...
				if (wand == null) {
					...
					final int newLevel = item.level() >= level() ? level() > 0 ? item.level() + 1 : item.level() : level();
					GameScene.show(new WndOptions("", Messages.get(MagesStaff.class, "warning", newLevel),
							Messages.get(MagesStaff.class, "yes"), Messages.get(MagesStaff.class, "no")) {
						@Override
	...
	private final Emitter.Factory StaffParticleFactory = new Emitter.Factory() {
		...
		public boolean lightMode() {
			return !((wand instanceof WandOfDisintegration) || (wand instanceof WandOfCorruption)
					|| (wand instanceof WandOfCorrosion) || (wand instanceof WandOfRegrowth)
