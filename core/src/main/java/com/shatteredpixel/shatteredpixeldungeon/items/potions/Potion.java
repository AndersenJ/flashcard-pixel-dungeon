...
public class Potion extends Anonymizable{
	...
	private static final Class<?>[] potions = { PotionOfHealing.class, PotionOfExperience.class, PotionOfToxicGas.class,
			PotionOfLiquidFlame.class, PotionOfStrength.class, PotionOfParalyticGas.class, PotionOfLevitation.class,
	...
	public void execute(final Hero hero, String action) {
		...
		if (action.equals(AC_CHOOSE)) {
			...
			if (isKnown() && mustThrowPots.contains(getClass())) {
				...
				GameScene.show(new WndOptions(Messages.get(Potion.class, "harmful"), Messages.get(Potion.class, "sure_drink"),
	...
	public void doThrow(final Hero hero) {
		...
		if (isKnown() && !mustThrowPots.contains(this.getClass()) && !canThrowPots.contains(this.getClass())) {

			GameScene.show(new WndOptions(Messages.get(Potion.class, "beneficial"), Messages.get(Potion.class, "sure_throw"),
					Messages.get(Potion.class, "yes"), Messages.get(Potion.class, "no")) {
				@Override
				protected void onSelect(int index) {
					if (index == 0) {
	...
	public static class SeedToPotion extends Recipe {
		...
		public Item brew(List<Item> ingredients) {
			...
			List<Class<? extends Plant.Seed>> seeds = new ArrayList<>();

			ingredients.stream().filter(ingredient -> !seeds.contains(ingredient.getClass()))
					.map(ingredient -> (Plant.Seed) ingredient).map(ingredient -> ingredient.getClass())
