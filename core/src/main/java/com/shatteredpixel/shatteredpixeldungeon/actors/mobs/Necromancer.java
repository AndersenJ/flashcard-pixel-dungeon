...
public class Necromancer extends Mob {
	...
	private class Hunting extends Mob.Hunting {
		...
		public boolean act(boolean enemyInFOV, boolean justAlerted) {
			...
			if (summoning) {
				...
				if (Actor.findChar(summoningPos) != null) {
					...
					for (int c : PathFinder.NEIGHBOURS8) {
						if (Actor.findChar(summoningPos + c) == null && Dungeon.level.passable[summoningPos + c]
			...
			if (mySkeleton != null
					&& (!mySkeleton.isAlive() || !Dungeon.level.mobs.contains(mySkeleton) || mySkeleton.alignment != alignment)) {
				mySkeleton = null;
			}

			// if enemy is seen, and enemy is within range, and we haven no skeleton, summon
			// a skeleton!
			if (enemySeen && Dungeon.level.distance(pos, enemy.pos) <= 4 && mySkeleton == null) {

				summoningPos = -1;
				for (int c : PathFinder.NEIGHBOURS8) {
					if (Actor.findChar(enemy.pos + c) == null && Dungeon.level.passable[enemy.pos + c]
