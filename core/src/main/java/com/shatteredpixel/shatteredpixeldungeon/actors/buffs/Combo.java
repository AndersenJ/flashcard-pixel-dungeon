...
public class Combo extends Buff implements ActionIndicator.Action {
	...
	private CellSelector.Listener finisher = new CellSelector.Listener() {
		...
		public void onSelect(Integer cell) {
			...
			if (enemy == null || !Dungeon.level.heroFOV[cell] || !((Hero) target).canAttack(enemy)
		...
		private void doAttack(final Char enemy) {
			...
			case CLEAVE:
				if (!enemy.isAlive()) {
					// combo isn't reset, but rather increments with a cleave kill, and grants more
					// time.
					hit(enemy);
					comboTime = 12f;
				} else {
		...
		public String prompt() {
			if (count >= 10)
				return Messages.get(Combo.class, "fury_prompt");
			else if (count >= 8)
				return Messages.get(Combo.class, "crush_prompt");
			else if (count >= 6)
				return Messages.get(Combo.class, "slam_prompt");
			else if (count >= 4)
				return Messages.get(Combo.class, "cleave_prompt");
			else
				return Messages.get(Combo.class, "clobber_prompt");
		}
	};
}
