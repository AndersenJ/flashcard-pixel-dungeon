...
public class Hero extends Char {
	...
	// performance
	public List<Mob> mindVisionEnemies = new ArrayList<>();
	...
	public boolean canSurpriseAttack() {
		...
		if (STR() < ((Weapon) belongings.weapon).STRReq())
			return false;
		if (belongings.weapon instanceof Flail)
			return false;
		...
		if (pos == dst) {
			...
			if (heap != null) {
				...
				if (item.doPickUp(this)) {
					...
					if (item instanceof Dewdrop || item instanceof TimekeepersHourglass.sandBag || item instanceof DriedRose.Petal
							|| item instanceof Key) {
						// Do Nothing
					} else {

						boolean important = (item instanceof ScrollOfUpgrade && ((Scroll) item).isKnown())
	...
	public void checkVisibleMobs() {
		...
		if (target != null && (lastTarget == null || !lastTarget.isAlive() || !fieldOfView[lastTarget.pos])
	...
	private boolean getCloser(final int target) {
		...
		if (Dungeon.level.adjacent(pos, target)) {
			...
			if (Actor.findChar(target) == null) {
				...
				if (walkingToVisibleTrapInFog && Dungeon.level.traps.get(target) != null
			...
			if (path == null || path.isEmpty() || !Dungeon.level.adjacent(pos, path.getFirst()))
				...
				// Note that this is shorter than for mobs, so that mobs usually yield to the
				// hero
		...
		if (step != -1) {
			...
			// really there should be a way for traps to specify whether action should
			// continue or
	...
	public boolean handle(int cell) {
		...
		if (Dungeon.level.map[cell] == Terrain.ALCHEMY && cell != pos) {
			...
			curAction = new HeroAction.Unlock(cell);
				...
				|| Dungeon.level.map[cell] == Terrain.UNLOCKED_EXIT) && Dungeon.depth < 26) {
			...
			if (!Dungeon.level.visited[cell] && !Dungeon.level.mapped[cell] && Dungeon.level.traps.get(cell) != null
	...
	public void die(Object cause) {
		...
		if (ankh != null && ankh.isBlessed()) {
			...
			// ensures that you'll get to act first in almost any case, to prevent reviving
			// and then instantly dieing again.
	...
	public static void reallyDie(Object cause) {
		...
		List<Item> items = new ArrayList<>(Dungeon.hero.belongings.backpack.items);
	...
	// effectively cache this buff to prevent having to call buff(Berserk.class) a
	// bunch.
	// This is relevant because we call isAlive during drawing, which has both
	// performance
	// and concurrent modification implications if that method calls
	// buff(Berserk.class)
