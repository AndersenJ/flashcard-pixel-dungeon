...
public class NewTengu extends Mob {
	...
	private void jump() {
		...
		if (Dungeon.level instanceof NewPrisonBossLevel) {
			...
			if (level.state() == NewPrisonBossLevel.State.FIGHT_START) {
				...
				} while (level.solid[newPos] || level.distance(newPos, enemy.pos) < 5 || level.distance(newPos, enemy.pos) > 7
						|| level.distance(newPos, pos) < 6 || Actor.findChar(newPos) != null
	...
	// *****************************************************************************************
	// ***** Tengu abilities. These are expressed in game logic as buffs, blobs, and
	// items *****
	// *****************************************************************************************
		...
		if (abilitiesUsed >= targetAbilityUses()) {
			...
			if (targetAbilityUses() - abilitiesUsed >= 4) {
				...
				if (abilityCooldown == -1)
					abilityCooldown = Random.IntRange(1, 4);
			}
	...
	public static boolean throwBomb(final Char thrower, final Char target) {
		...
		((MissileSprite) thrower.sprite.parent.recycle(MissileSprite.class)).reset(thrower.sprite, finalTargetCell, item,
				new Callback() {
					@Override
					public void call() {
	...
	public static boolean throwShocker(final Char thrower, final Char target) {
		...
		// Targets closest cell which is adjacent to target, and not adjacent to thrower
		// or another shocker
		for (int i : PathFinder.NEIGHBOURS8) {
			int cell = target.pos + i;
		...
		public boolean act() {
			...
			if (shockingOrdinals == null) {
				...
				target.sprite.parent
						.add(new Lightning(shockerPos - 1 - Dungeon.level.width(), shockerPos + 1 + Dungeon.level.width(), null));
				target.sprite.parent
						.add(new Lightning(shockerPos - 1 + Dungeon.level.width(), shockerPos + 1 - Dungeon.level.width(), null));
				...
				target.sprite.parent
						.add(new Lightning(shockerPos - Dungeon.level.width(), shockerPos + Dungeon.level.width(), null));
