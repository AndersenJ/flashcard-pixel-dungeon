...
public abstract class Mob extends Char {
	...
	protected Char chooseEnemy() {
		...
		// if we are an enemy, and have no target or current target isn't affected by
		// aggression
		// then auto-prioritize a target that is affected by aggression, even another
		// enemy
		if (alignment == Alignment.ENEMY && (enemy == null || enemy.buff(StoneOfAggression.Aggression.class) == null)) {
		...
		if (newEnemy) {
			...
			if (enemies.isEmpty()) {
				...
				// go after the closest potential enemy, preferring the hero if two are
				// equidistant
				Char closest = null;
	...
	protected boolean getCloser(int target) {
		...
		if (Dungeon.level.adjacent(pos, target)) {
			...
			// location
			// or if it's extremely inefficient and checking again may result in a much
			// better path
			...
			// FIXME this is fairly brittle, primarily it assumes that hunting mobs can't
			// see through
			// permanent terrain, such that if their path is inefficient it's always because
			// of a temporary blockage, and therefore waiting for it to clear is the best
			// option.
	...
	protected boolean getFurther(int target) {
		...
		int step = Dungeon.flee(this, pos, target, Dungeon.level.passable, fieldOfView);
		if (step != -1) {
			move(step);
			return true;
		} else {
	...
	public int defenseSkill(Char enemy) {
		...
		if (enemy == Dungeon.hero && !Dungeon.hero.canSurpriseAttack())
			seen = true;
		if (seen && paralysed == 0 && !(alignment == Alignment.ALLY && enemy == Dungeon.hero)) {
	...
	public int defenseProc(Char enemy, int damage) {
		...
		if ((!enemySeen || enemy.invisible > 0) && enemy == Dungeon.hero && Dungeon.hero.canSurpriseAttack()) {
	...
	private static List<Mob> heldAllies = new ArrayList<>();
