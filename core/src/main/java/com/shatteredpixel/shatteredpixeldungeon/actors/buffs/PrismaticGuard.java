...
public class PrismaticGuard extends Buff {
	...
	public boolean act() {
		...
		for (int i = 0; i < v; i++) {
			...
			if (mob.isAlive() && mob.state != mob.PASSIVE && mob.state != mob.WANDERING && mob.state != mob.SLEEPING
					&& !hero.mindVisionEnemies.contains(mob) && (closest == null
							|| Dungeon.level.distance(hero.pos, mob.pos) < Dungeon.level.distance(hero.pos, closest.pos))) {
				closest = mob;
			}
		}
		...
		if (closest != null && Dungeon.level.distance(hero.pos, closest.pos) < 5) {
			...
			for (int i = 0; i < PathFinder.NEIGHBOURS8.length; i++) {
				...
				if (Actor.findChar(p) == null && Dungeon.level.passable[p]) {
					if (bestPos == -1
							|| Dungeon.level.trueDistance(p, closest.pos) < Dungeon.level.trueDistance(bestPos, closest.pos)) {
						bestPos = p;
					}
				}
			}
