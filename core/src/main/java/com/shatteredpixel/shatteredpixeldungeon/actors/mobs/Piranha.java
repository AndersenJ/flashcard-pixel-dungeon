...
public class Piranha extends Mob {
	...
	public int defenseSkill(Char enemy) {
		enemySeen = state != SLEEPING && this.enemy != null && fieldOfView != null && fieldOfView[this.enemy.pos]
	...
	protected boolean getCloser(int target) {
		...
		int step = Dungeon.findStep(this, pos, target, Dungeon.level.water, fieldOfView);
		if (step != -1) {
			move(step);
			return true;
		} else {
	...
	protected boolean getFurther(int target) {
		int step = Dungeon.flee(this, pos, target, Dungeon.level.water, fieldOfView);
		if (step != -1) {
			move(step);
			return true;
		} else {
