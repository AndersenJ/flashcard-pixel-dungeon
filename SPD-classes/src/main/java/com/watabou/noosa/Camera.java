...
public class Camera extends Gizmo {
	...
	private static List<Camera> all = new ArrayList<>();
	...
	PointF panTarget;
	// camera moves at a speed such that it will pan to its current target in
	// 1/intensity seconds
	// keep in mind though that this speed is constantly decreasing, so actual pan
	// time is higher
	...
	protected void updateMatrix() {
		...
		/*
		 * Matrix.setIdentity( matrix ); Matrix.translate( matrix, -1, +1 );
		 * Matrix.scale( matrix, 2f / G.width, -2f / G.height ); Matrix.translate(
		 * matrix, x, y ); Matrix.scale( matrix, zoom, zoom ); Matrix.translate( matrix,
		 * scroll.x, scroll.y );
		 */
