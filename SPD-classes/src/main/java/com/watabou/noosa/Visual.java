...
public class Visual extends Gizmo {
	...
	@Override
	// TODO caching the last value of all these variables does improve performance a
	// bunch
	// by letting us skip many calls to updateMatrix, but it is quite messy. It
	// would be better to
	// control their editing and have a single boolean to tell if the matrix needs
	// updating.
	public void draw() {
		if (lastX != x || lastY != y || lastW != width || lastH != height || lastA != angle || lastScale.x != scale.x
				|| lastScale.y != scale.y || lastOrigin.x != origin.x || lastOrigin.y != origin.y) {
	...
	// color must include an alpha component (e.g. 0x80FF0000 for red at 0.5
	// strength)
