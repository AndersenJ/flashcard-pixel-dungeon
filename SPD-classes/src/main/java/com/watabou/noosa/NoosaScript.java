...
public class NoosaScript extends Script {
	...
	public void drawQuadSet(Vertexbuffer buffer, int length, int offset) {
		...
		Gdx.gl20.glDrawElements(GL20.GL_TRIANGLES, Quad.SIZE * length, GL20.GL_UNSIGNED_SHORT,
				Quad.SIZE * Short.SIZE / 8 * offset);
	}
	...
	public void camera(Camera camera) {
		...
		if (camera != lastCamera && camera.matrix != null) {
			...
			if (!camera.fullScreen) {
				...
				Gdx.gl20.glScissor(camera.x, Game.height - camera.screenHeight - camera.y, camera.screenWidth,
						camera.screenHeight);
			} else {
